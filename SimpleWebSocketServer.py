...
class BroadcasterWebsocketServer(Thread):
    ...
    def __init__(self, host, port, debugInfo=False):
        ...
        self.server = SimpleWebSocketServer(host, port, SimpleChat)
...
def recvLogcat(server):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.connect(('192.168.57.101', 11229))
    with sock.makefile('r', buffering=4096) as f:
        for line in f:
            #print(line)
...
def startServer():
    server = BroadcasterWebsocketServer('', 8000, True)
    server.start()
    while True:
        try:
            recvLogcat(server)
